/*
 * $Id: AsyncHttpRequest.java 306 2009-11-04 14:18:07Z fabriziogiudici $
 *
...
# property to follow redirects*/

/**
 * <p>A simple non visual bean that uses HTTP to interact with web servers. This
 * class conforms (except for XML specific functionality) with XMLHttpRequest
 * as defined in the W3C Working Draft Specification
 * <a href="http://www.w3.org/TR/XMLHttpRequest/">http://www.w3.org/TR/XMLHttpRequest/</a>.</p>
 *
 * <p>Several methods, such as the abort() method, cause the AsyncHttpRequest to be
 * "reset". This means that any state associated with the response is cleared,
 * and the readyState reset to UNINITIALIZED.</p>
 *
 * <p>AsyncHttpRequest relies on {@link org.jdesktop.http.Session} for communicating
 * with the server. It is possible to reuse an existing Session. However, if a
 * Session is not specified, a new one is constructed for the request.</p>
 *
 * @author rbair
 */
public class AsyncHttpRequest extends AbstractBean {
    ...
    // ---------------------------------------------------------- Properties
    /**
     * The ReadyState of this AsyncHttpRequest.
     */
     ...
     * of the entity body received so far (when readyState is RECEIVING) or
     * the complete entity body (when readyState is LOADED), interpreted as a
     ...
     * It MUST be available when readyState is RECEIVING or LOADED. When available,
     * it MUST represent the HTTP status text sent by the server
     */
    ...
    /**
     * Flag used to indicate whether the task should be run asynchronously or not.
     * This is reset in the open() method.
     */
    private boolean asyncFlag;
    private Session s;
    private Exception exception;
    private PropertyChangeListener onReadyStateChange;
    private Request req = new Request(); //the request. Reset in reset(). Never null.
     ...
     *
     * @param s the Session to use. This may be null. If null, then a default
     ...
     * This is functionally equivilent to adding the listener to the
     * list of listeners via #addReadyStateChangeListener. This method
     * exists soley for compliance with the spec.
     *
    ...
    /**
     * Returns the ready-state change listener. This method returns whatever
     * is set via the #setOnReadyStateChange method. If listeners were added
     * via #addReadyStateChangeListener, they are not reflected in this method.
     *
    ...
    /**
     * <p>Gets the ready state of this AsyncHttpRequest.</p>
     *
     * <p>Ready state will be one of the following values:
     * <ul>
    ...
    private void open(Method method, String url, boolean asyncFlag, String username) {
        open(method, url, asyncFlag, username, null);
    }
    
    /**
     * Initializes the AsyncHttpRequest prior to sending a request. Calling this method
     * initializes AsyncHttpRequest by storing the method, uri, asyncFlag, userName, and password
     * values. This also sets the readyState to OPEN, resets the response state to
     * their initial values, and resets the request headers.
     *
     * If open() is called when readyState is LOADED, then the entire object is reset.
     * (huh?)
     *
     * NOTE: Private because authentication is not yet implemented (and indeed,
     *       I don't know yet how to implement it.)
     ...
     * @param method
     * @param uri
     * @param asyncFlag Defaults to "true"
     * @param userName Defaults to null
     * @param password Defaults to null
     */
    private void open(Method method, String url, boolean asyncFlag, String username, String password) {
        ...
        //check the url to ensure it is a valid URL
        try {
            new URL(url);
        } catch (Exception e) {
     ...
     * other than OPEN, then an IllegalStateException will be thrown. At the beginning
     * of this method the readyState will be set to SENT. If the async flag is set
     * to false, then the method will not return until the request
     ...
     * et etc
     *
     * Immediately before receiving the message body (if any), the readyState attribute
     * will be changed to RECEIVING. When the request has completed loading, the
     * readyState attribute will be set to LOADED. In case of a HEAD request, readyState
     * will be set to LOADED immediately after having been set to RECEIVING.
     */
    public void send() {
        send((String)null);
    }
    ...
    public void send(String content) {
        if (readyState != ReadyState.OPEN) {
            throw new IllegalStateException("AsyncHttpRequest must be in an OPEN state before " +
    ...
    /**
     * <p>Gets all of the response headers as one long string.</p>
     *
     * <p>If the readyState property has a value other than RECEIVING or LOADED, this
     * method will return null. Otherwise, it will return all the HTTP headers as
     * a single string, with each header line separated by a CR/LF pair. The status
     * line will not be included.</p>
     *
    ...
    /**
     * <p>Gets a single response header as a string.</p>
     *
     * <p>If the readyState property has a value other than RECEIVING or LOADED, this
     * method will return null. Otherwise, it will represent the value of the
     * given HTTP header in the data received so far from the last request sent,
     * as a single string. If more than one header of the given name was received,
     * then the values will be concatenated, separated from each other by a comma
     * followed by a single space. If no headers of that name were received, then
     * it will return the empty String.</p>
     *
     * @param headerLabel the label of the response header to retreive.
    ...
    public String getStatusText() {
        if (readyState == ReadyState.RECEIVING || readyState == ReadyState.LOADED) {
            ...
            throw new IllegalStateException("You cannot call getStatusText() unless" +
                    " readyState == RECEIVING || LOADING");
        }
    }
    ...
    /**
     * Specifies whether the request should automatically follow redirects. By default,
     * this is set to false.
     *
     * @param flag indicates whether to follow redirects automatically
     */
    ...
    public void setTimeout(long timeout) {
        if (readyState != ReadyState.OPEN) {
            throw new IllegalStateException("The request must be OPEN before setting the timeout");
        }
    ...
    */
     ...
     * These are reset whenever this AsyncHttpRequest is reset.
     * 
    ...
    public void addAsyncRequestListener(AsyncRequestListener listener) {
        listenerList.add(AsyncRequestListener.class, listener);
    }
    
    public void removeAsyncRequestListener(AsyncRequestListener listener) {
        listenerList.remove(AsyncRequestListener.class, listener);
    }
    
    public AsyncRequestListener[] getAsyncRequestListeners() {
        return listenerList.getListeners(AsyncRequestListener.class);
    }*/
     ...
     * Any overriding implementations MUST call super.reset() at the end of the
     * implementation.
     */
    ...
    /**
     * Method that provides a hook for subclasses to create concrete types
     * (such as DOM, JSONObject, etc) when the response has been fully read.
     * There is no need to call super.handleResponse(txt).
     */
     ...
     * ensures that the readyState property change event is fired. This MUST
     ...
     * ensures that the status property change event is fired. This MUST
     * be called on the EDT.
     */
     ...
     * ensures that the statusText property change event is fired. This MUST
     * be called on the EDT.
     */
    ...
    protected class AsyncWorker extends SwingWorker {
        ...
        protected Object doInBackground() throws Exception {
            try {
                //TODO!!!
                //connection timeout
                //another thread is used (java.util.Timer) to keep track of a timeout.
                ...
                //TODO!!! Need to see if there is a way to set to RECEIVING when the first bit of data
                //comes down, instead of waiting until the whole thing is read (which is what I THINK
                //is happening here).
                safeSetReadyState(ReadyState.RECEIVING);

                //grab the resulting data
                responseText = response.getBody();
                handleResponse(responseText); //causes the cached version of the string to be created
        ...
        /**
         * Helper method which allows me to set the ready state on
         * the EDT
         */
